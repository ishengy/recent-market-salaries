...
def hello():
    return render_template(
        "landing.html", job_list=test_df.job_title.unique(), loc_list=col_adj.City
    )
...
def get_plot():
    ...
    if request.method == "POST":
        ...
        print(experience)
        adj_factor = (
            col_adj[col_adj.City == location]["Cost of Living Index"].values[0] / 100
        )
        ...
        plt.title(
            "Salary Distribution \n (Source: LinkedIn, Sample Size: "
            + str(int(data_row.num_salary.values[0]))
            + ")"
        )
        plt.ylabel("Density")
        plt.xlabel("Salary")
        ...
        output_compare = (
            "This means that your salary falls in the "
            + str(ceil(percentile * 100))
            + "th percentile."
        )
        non_nyc_warning = ""
        if location != "New York, NY, United States":
            non_nyc_warning = (
                "This plot is created by adjusting against NYC Cost of Living; it is not an accurate representation of the salaries in "
                + location
                + "."
            )
